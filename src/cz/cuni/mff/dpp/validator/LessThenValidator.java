 ...
 */
public class LessThenValidator<T extends Comparable<T>> extends AbstractValidator<T> {

    private final T constraint;

    /**
     * Creates a validator, that validates that the argument is less than the given constraint.
     * 
     * @param targetClass
     *            the class of the argument to validate
     * @param constraint
     *            the argument must be less than this value, or else the validation fails.<br>
     *            The array must contain exactly one value
     */
    public LessThenValidator(final Class<T> targetClass, final T[] constraint) {
        super(targetClass);
